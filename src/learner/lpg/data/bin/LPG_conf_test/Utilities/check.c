...
/*verifica tempo impiegato*/
  ...
  if ((gcmd_line.max_optimization_cpu_time != 0 && GpG.num_solutions )
      && (elapsed_time >= gcmd_line.max_optimization_cpu_time))
    {
      printf ("\nWarning: max optimization cpu-time reached\n\n");
      exit (1);
    }
  return 1;
}
...
{
  int level, pos, ltrue, error, temp;
  ...
  for (level = 0; level <= max_time && level < GpG.max_plan_length; level++)
	...
	if (CHECK_FACT_POS (pos, level))

	  {
	    ...
	    ltrue = (vectlevel[level]->fact_vect[uid_block] & uid_mask);
	    if ((fact->w_is_true != 0) != (ltrue != 0))
	      printf ("\nFact error %s w_is_true %d bit %d  level %d position %d",
		      print_ft_name_string (pos, temp_name), fact->w_is_true,
		      ltrue, level, pos);
	    ltrue = vectlevel[level]->prec_vect[uid_block] & uid_mask;
	    if ((fact->w_is_goal > 0) != (ltrue != 0))
	      printf ("\nFact error %s w_is_goal %d bit %d level %d ",
		      print_ft_name_string (pos, temp_name), fact->w_is_goal,
		      ltrue, level);
	    ltrue = vectlevel[level]->false_crit_vect[uid_block] & uid_mask;
	    if ((fact->w_is_goal && fact->w_is_true == 0) != (ltrue != 0))
	      printf
		("\nFact error False critic %s w_is_goal %d w_is_true %d bit %d  level %d",
		 print_ft_name_string (pos, temp_name), fact->w_is_goal,
		 fact->w_is_true, ltrue, level);
	    ltrue = vectlevel[level]->true_crit_vect[uid_block] & uid_mask;
	    if ((fact->w_is_goal && fact->w_is_true == 1) != (ltrue != 0))
	      printf
	...
	if (CHECK_NOOP_POS (pos, level))

	  {
	    ...
	    uid_block = GUID_BLOCK (pos);
	    uid_mask = GUID_MASK (pos);
	    ltrue = vectlevel[level]->noop_act_vect[uid_block] & uid_mask;
	    if ((noop->w_is_used > 0) != (ltrue != 0))
	      printf ("\nNoop error %s w_is_used %d bit %d  level %d",
		      print_noop_name_string (pos, temp_name), noop->w_is_used,
		      ltrue, level);
	    ltrue = vectlevel[level]->noop_prec_act_vect[uid_block] & uid_mask;
	    if ((noop->w_is_goal > 0) != (ltrue != 0))
	      printf ("\nNoop error %s w_is_goal %d bit %d  level %d",
		      print_noop_name_string (pos, temp_name),
		      noop->w_is_goal, ltrue, level);
	    if (noop->w_is_used
	  ...
	  for (i = 0; i < gnum_comp_var; i++)
	    {
	      ...
	      //Check unsatisfied numeric goals
	      if (vectlevel[level]->numeric->w_is_used[i])
		{
		  if(!is_num_prec_satisfied(i,level) && vectlevel[level]->numeric->false_position[i] < 0)
		    {
#ifdef __MY_OUTPUT__
		      printf("\n Error unsup numfact %d lev %d step %d", i, level, GpG.count_num_try);
#endif
		      insert_unsup_numeric_fact (i, level);
		    }
		}
	    }
		  ...
		  for (i = 0; i < gef_conn[position].sf->num_PC_overall; i++)
		    {
		      ...
		      if (el < 0)
			{
			  if(!is_num_prec_satisfied_after_start(-el,level) && vectlevel[level]->numeric->false_position[-el] < 0)
			    {
#ifdef __MY_OUTPUT__
...
eval_comp_var_non_recursive_bet (int cv_index, float *in_vect, float *out_vect)
{
  ...
  switch (gcomp_var[cv_index].l_operator)
    {
	 ...
	 if(gef_conn[act_num].is_numeric)	 
	   for(i=0;i<gef_conn[act_num].num_numeric_effects;i++)
	       ...
	       if ((*numeric_effect).is_at_start)
		 {
		   ...
		   if(gcomp_var_effects[(*numeric_effect).index].l_operator==INCREASE_OP)
		   printf("INCREASE");
		   else 
		   printf("DECREASE");
		   ...
		   if ((*numeric_effect).is_at_start)
		   printf("E' at start");
		   printf(" \nTipo var1: %d  FIRTS_OP: %d SECOND_OP: %d  Tipo var2: %d FIRST_OP:%d SECOND_OP:%d", gcomp_var[gcomp_var_effects[(*numeric_effect).index].first_op].l_operator,gcomp_var[gcomp_var_effects[(*numeric_effect).index].first_op].first_op,gcomp_var[gcomp_var_effects[(*numeric_effect).index].first_op].second_op,
		   gcomp_var[gcomp_var_effects[(*numeric_effect).index].second_op].l_operator,gcomp_var[gcomp_var_effects[(*numeric_effect).index].second_op].first_op,gcomp_var[gcomp_var_effects[(*numeric_effect).index].second_op].second_op );
		   ...
		   gcomp_var_effects[(*numeric_effect).index].l_operator);
...
#ifndef __ONLY_ONE_PLANNER__
	    printf("\n\nIn order to find better solutions, please use option -n.\n\n");
	    exit(0);
#endif
	  }
      }
	...
	if (GpG.max_cputime_for_local_search>0.0 || GpG.max_cputime > 0.0) // facciamo partire il BEST FIRST solo se non si e' ancora trovata una soluzione
	  {
	    ...
	    if ((gcmd_line.max_optimization_cpu_time != 0 && GpG.num_solutions )
		&& ((gtotal_time- GpG.first_solution_cpu_time) >= gcmd_line.max_optimization_cpu_time))
	      {
		printf ("\nWarning: max optimization cpu-time exceeded: %.2f \n\n",gtotal_time );
		exit (1);
	      }
...
Bool check_numeric_w_is_goal(void)
{
  ...
  for (lev = 0; lev < GpG.curr_plan_length; lev++)
    {
      for (el = 0; el < gnum_comp_var; el++)
	{
	  if (vectlevel[lev]->numeric->w_is_goal[el] > 0)
	    {
	      //printf("\nCheck var %d, lev %d", el, lev);
	      
	      count =  vectlevel[GpG.curr_plan_length]->numeric->w_is_goal[el];
	      for (l = lev; l < GpG.curr_plan_length; l++)
		{

		  if (is_fact_in_preconditions(GET_ACTION_POSITION_OF_LEVEL(l), -el)
		      || is_fact_in_preconditions_overall(GET_ACTION_POSITION_OF_LEVEL(l), -el)
		      || is_fact_in_preconditions_end(GET_ACTION_POSITION_OF_LEVEL(l), -el))
		    {
		      //printf("\nGoal in level %d", l);
		      count++;
		    }
		  
		  if (check_numeric_precond_interference(el, l))
		    break;
		} 
	      if (count != vectlevel[lev]->numeric->w_is_goal[el])
		{
		  res = FALSE;
		  printf("\nError: w_is_goal for numeric var %d count %d, should be %d [level %d]\n", el, 
			 vectlevel[lev]->numeric->w_is_goal[el], count, lev);
		  printf("fact: %s", print_ft_name_string(-el, temp_name));
		}
	    }
	}
    }
