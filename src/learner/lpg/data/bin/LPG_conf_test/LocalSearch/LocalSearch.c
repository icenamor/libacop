...
float fast_action_cost (neighb_list neighb_act)
{
  ...
  **/
  ...
  /** 
    ...
    **
    It determines the number of not supported preconditions
  **/
  if (prec_par)
    {
  ...
  /** 
    ...
    **
    It determines the number of actions ME with the action given as input
  **/
  ...
  /** 
      ...
      **
      ...
      a fact is a true critic node if it is precondition of almost an action of the upper levels and it's 
      supported from only one action 
      a fact is a false critic node if it is precondition of almost an action of the upper levels and it's 
      not supported
  **/
  if (add_effect_par)
    {
	...
	ptr = vectlevel[level]->true_crit_vect;	/** 
						    ...
						    **
						    Number of critic facts that could become false 
						**/
      else
	ptr = vectlevel[level]->false_crit_vect;/** 
						    ...
						    Aspetto positivo
						    **
						    Number of critic facts that could become true;
						    positive aspect 
						**/
      if (neighb_act->constraint_type == C_T_REMOVE_ACTION
	  && !GpG.accurate_cost)
...
float action_cost (neighb_list Action)
{
  /** 
      ...
      **
      I copy the values of the action's level
  **/
  ...
  /** 
    In base al tipo di piano determino come calcolare il costo relativo all'inserimento o rimozione di una 
    azione 
    **
    Depending to the type of the plan I determine the mode of calculating the cost relative to the 
    insertion or remotion of one action
  **/
...
**
*  Name: action_eff_cost
*  Objective: count the number of false critical additive effects
*  Type: int
...
int action_eff_cost (register ActNode_list infAction)
{
  ...
  **/
  ...
  /** 
    ...
    **
    Get the level from the structure inf_action in input
  **/
  ...
  **/
  ...
  /** 
    Se l'azione e' inserita nell'action subgraph associo a ptr il numero di fatti critici che possono 
    diventare falsi 
    **
  ...
  **/
  if (infAction->w_is_used)
    ptr = vectlevel[level]->true_crit_vect;  /** 
						Numero di fatti critici che possono diventare falsi
						**
					        Number of critics fact that could become false 
					     **/
  /** 
  ...
  **/
  ...
  /** 
    ...
    **
    It analyzes the additive effects and increases temp of the number of critical facts
  **/
  for (i = 0, temp = 0; i < act->num_add_effect; i++)
    temp +=
  ...
  for (el = ilist; el; el = el->next)
      ...
      else
	{
	  if (gef_conn[el->item].level < 0)
	    {
	      if (DEBUG2)
		printf("\nWarning: trying to insert a non reachable action in neighborhood!");
...
**
*  Name: find_min
*  Objective: to order the neighbors vector in cost increasing order
*  Type: float
	  ...
	  int *num_min, int *num_neg)
{
  ...
  float cost = 0.0, minor_cost = MAXFLOAT , temp_c;
  float max_act_incons, max_act_cost, max_act_time, max_timed_fa,max_input_plan_cost, weight, weight_c, weight_t, weight_tf,weight_ip, diff;
  node_cost n_cost;
  ...
  max_act_time = 0.0;
  max_timed_fa = 0.0;
  max_input_plan_cost=0.0;

  weight = weight_c = weight_t = weight_tf = weight_ip = 0.0;
  
  /** 
    Pongo alcuni campi di local_search uguali a MAXFLOAT, mi servono  per limitare la 
    profondita' di ricerca 
    **
    I set some fields of local_search equal to MAXFLOAT, needed to limit the
    search depth
  **/
  ...
  **/
  ...
  /** 
    ...
    **
    I analyze the number of neighbors
  **/
  for (i = num - 1; i >= 0; i--)
    {
	...
	**
	I update the cost of the action
      **/
      (*(neighb_vect + i))->cost.timed_fa = (*(neighb_vect + i))->cost.weight =
	(*(neighb_vect + i))->cost.act_cost = (*(neighb_vect + i))->cost.act_time = (*(neighb_vect + i))->cost.input_plan_cost= 0.0;
	...
	dell'azione 
	**
	If the search cost is smaller than the maximum (max_act_incons) I update this with that one
	of the action
      **/
	...
	**
	If the cost of the insertion or remotion is smaller than the maximum (max_act_cost) I update
	...
	**
	If is the first action considered I simply update the fields of local_search
      **/
      if(max_timed_fa < (*(neighb_vect + i))->cost.timed_fa)
	max_timed_fa = (*(neighb_vect + i))->cost.timed_fa;

      /* Update adaptationmax cost */
      if(max_input_plan_cost  < (*(neighb_vect + i))->cost.input_plan_cost)
	max_input_plan_cost = (*(neighb_vect + i))->cost.input_plan_cost;
      ...
      if(GpG.weight_cost <= 0 && GpG.weight_time <= 0 && GpG.timed_facts_present==FALSE)
	{
	  /** 
	    ...
	    **
	    Controls on the search cost, on the insertion cost and on the temporal cost
	  **/
      ...
      else
	{
	  ...
	  **/
	  ...
	  /** 
	    ...
	    **
	    Otherwise I analyze the single situations
	  **/
...
#endif
  if (  GpG.weight_input_plan_cost >0 && max_input_plan_cost  )
       weight_ip = GpG.weight_input_plan_cost / max_input_plan_cost;
  ...
  if(GpG.evaluation_function==1 || GpG.evaluation_function==2 )
	...
	weight_tf = GpG.weight_timed_fa * max_timed_fa;
     ...
     // ALEX: (aggiunta di GpG.curr_goal_state->num_F) In caso non ci
     // siano goals siamo gia' nel caso in cui e' importante la
     // qualita'
     if(GpG.num_false_tot>0 && GpG.evaluation_function==1 && GpG.curr_goal_state->num_F)
       {
	  ...
	  if(GpG.evaluation_function==0)
	    {
	      ...
	      cost = max_act_incons * ((*(neighb_vect + i))->cost.act_cost * weight_c + (*(neighb_vect + i))->cost.act_time * weight_t ) + (*(neighb_vect + i))->cost.weight + (*(neighb_vect + i))->cost.timed_fa * weight_tf ;
	        if( weight_ip >0 && (GpG.input_plan==TRUE))
		  cost +=  (*(neighb_vect + i))->cost.input_plan_cost * weight_ip ;

	    }
	  else
	    {
	     /**
		Moltiplico la componente dei costi per max_act_incons invece che dividere la componente del
		costo di ricerca poiche' la moltiplic e' + veloce ed evito probl di divisione per 0
		**
		I multiply the cost component per max_act_incoms instead to divide the research cost 
		component because the multiplication is faster and avoid problem with the division by 0
	     **/
	      ...
	      if( weight_ip >0 && (GpG.input_plan==TRUE))
		{
		  ...
		  temp_c = (*(neighb_vect + i))->cost.input_plan_cost * weight_ip  ;

		  if(GpG.weight_num_solutions==2 &&GpG.num_false_tot>0 )
		    temp_c=(temp_c)/ GpG.num_false_tot;
		  else

		  if(GpG.weight_num_solutions==1 &&GpG.num_false_tot>0&& GpG.num_solutions > 0 )
		    temp_c=(temp_c*GpG.num_solutions)/ GpG.num_false_tot;
		  else
		  if(GpG.num_false_tot>0 && GpG.num_solutions < 1 )
		    {
		      temp_c/=GpG.num_false_tot;

		    }
#ifdef __TEST_ADAPT__
		  printf("\nstep %d act %d cost %f adapt cost %f ipc %f weight_ip %f ",GpG.count_num_try,(*(neighb_vect + i))->act_pos, cost, temp_c,  (*(neighb_vect + i))->cost.input_plan_cost,weight_ip);
#endif
		  cost += temp_c;
		}
	  ...
	  if (( minor_cost- cost) >  MIN_VALUE )
	    {
		  ...
		  **
		  I place the action in the neighbors array in position i
	      **/
	      *pos_temp_vect = i;
	      /** 
	      ...
	      **/
	      local_search.best_cost = cost;	/** 
						    ...
						    azioni successive
						    **
						    Used in the phase of definition of the cost of the 
						    actions
						**/
	    }
	      ...
	      for (j = 0; j < pos; j++)
		{
		  ...
		  **
		  I place the actions (with equal cost) in the same position in the neighbors array
	      **/
	      if (j == pos)
		*(pos_temp_vect + pos++) = i;
	    }
	...
	**
      ...
      else if (cost <= 0.0)
	{
	  /** 
	    ...
	    **
	    If the action cost is smaller than minor_cost 
	  **/
	  if (cost < minor_cost)
	    {
	      pos = 1;
		...
		**
		I place the action in position in the neighbors array
	      **/
	      *pos_temp_vect = i;
	      /** 
		...
		**
		I update the field relative to the cost of local search (best_cost) with the cost of the 
		action
	      **/
	      local_search.best_cost = cost; /** 
						 Utilizzata nella fase di definizione del costo delle 
	  ...
	  **/
		...
		Check aggiuntivo per non inserire le azioni "simili" con lo stesso costo (cost)
		**
		Additional control to not insert similar actions with the same cost (cost)
	      **/
	      for (j = 0; j < pos; j++)
		{
		  if (j >= MAX_MAX_NODES){
  ...
  *num_min = pos;		/* numerosity of actions with lower cost */
  ...
  if( *num_min >1 && minor_cost<MAXFLOAT && GpG.extended_effects_evaluation)
	 ...
	 **
	 Trying to improve the euristic evaluation
      **/
      for(i=0; i<  *num_min ; i++)
	{
...
*  Call gives:
**/
float check_value (float new_v)
{
  if (new_v < MIN_POS_S_S)
    {
      // printf("L"); 
      return MIN_POS_S_S;
    }
  else if (new_v > MAX_POS_S_S)
    {
      // printf("M");
      return MAX_POS_S_S;
    }
...
/**  OK 27-07-04
 * Name: update_precond
 * Scopo: Scandisce tutta la lista delle precodizioni non supportate e aggiorna i moltiplicatori di 
 *        lagrange delle azioni che sono associate a tali precondizioni
 * Tipo: void
 * Input: nessuno
 * Output: nessuno
...
**
*  Name: update_precond
*  Objective: Scan the "not supported" precondition list and update the lagrange multipliers of actions
*             associated to each precondition
*  Type: void
  ...
  **/
  for (false_fa_pos = 0; false_fa_pos < GpG.num_false_fa; false_fa_pos++) {
	 ...
	 /**
	    ...
	    **
	    If the precondition is in the last level, it means that is a precondition of a goal, then the 
	    Lagrange multiplier of the fictous action memorized in GpG.goal_lambda is updated. Then is 
	    forced the forwarding return in the cycle
	 **/
...
**
*  Name: update_mutex
*  Objective: Update the Lagrange multipliers for the mutex defined by the action act
*  Type: void
*  Input: int pos_action
*  Output: None
...
**
*  Name: update_decr_me_prec
*  Objective: Decrease the Lagrange multipliers that are not increased in the same flips
*  Type: void
*  Input: none
*  Output: None
*  Main Data Structures:
...
**
*  Name: update_precond_multilevel
*  Objective: Scan the "not supported" precondition list and update the Lagrange multipliers for the action
*             associated to each precondition
*  Type: void
*  Input: none
*  Output: None
...
void update_precond_multilevel ()
{
  ...
  **/
  for (false_fa_pos = 0; false_fa_pos < GpG.num_false_fa; false_fa_pos++){
	 ...
	 GpG.goal_lambda. Viene forzato il ritorno anticipato nel ciclo con continue.
	 **
	 If the precondition is in the last level, it means that is a precondition of a goal, then is 
	 update the Lagrange multiplier of the fictious action stored in GpG.goal_lambda. Then is forced 
	 the forwarding return to the cycle
      **/
      if (*inconsistenza->level==GpG.curr_plan_length){
	   GpG.goal_lambda = check_value(GpG.goal_lambda+step_prec_incr);
	   GpG.flag_decr_lm_goal=1;
...
**
*  Name: update_decr_me_prec_multilevel
*  Objective: Decrease Lagrange multipliers that are not increased in the same flip
*  Type: void
*  Input: none
*  Output: None
*  Main Data Structures:
...
**
*  Name: update_mutex_multilevel
*  Objective: Update Lagrange multipliers for mutex
*  Type: void
*  Input: none
*  Output: None
*  Main Data Structures:
...
int not_tabu (int act)
{
  ...
  **/
  ...
  /** 
    Controlla che la ricerca non sia locale 
    **
    It check if the search is not local
  **/
  if(!GpG.tabuplan_act)
    return TRUE;
...
int define_neighborhood_for_threats (register NoopNode_list node_tofix, int initialize)
{
  ...
  /**
     ...
     **
     removing the inconsistence from the constraints array
  **/
  ...
  **/
  ...
  /**
     ...
     **
     check if the fact is precondition for at least one action
  **/
  ...
  **/
  ...
  /**
     ...
     **
     I choose an action that has the fact as precondition and the action that makes supported the fact
  **/
  for (i = *node_tofix->level + 1; i < GpG.curr_plan_length; i++)
	...
	/**
	   ...
	   **
	   Inserting the action in the neighborhood
	**/
...
int define_neighborhood (register FctNode_list node_tofix, int initialize)
{
  ...
  while (level < GpG.curr_plan_length && initialize > 0)
	 ...
	 **
	 I consider the actions that has this fact as precondition
      **/
      propagation = 1;
	  ...
	  && inf_action->w_is_used && not_tabu (el))
	{
	  ...
	  propagation = 0;     /**
				  ...
				  **
				  I only consider an action that has this fact as precondition
			       **/
	}
  ...
  if (  initialize > 0 && level > 0)
    {
      /**
	 Inseriamo l'eventuale azione che blocca la catena di noop che supporterebbe l'inconsistenza in 
	 questione
	 **
      ...
      if (level >= 0)
	{
	  ...
	  if (vectlevel[level]->fact[node_tofix->position].w_is_true
	      && not_tabu (el))
	    {
	      ...
	      insert_element_in_neighb (&temp_act);
	      propagation = 0;     /**
				      Considero unicamente una azione che ha questo fatto come 
				      precondizione
				      **
				      I only consider an action that has this fact as precondition
				   **/
  ...
  /**
     considero le azioni del livello *inform_tofix->level che rendono vero inform_tofix poiche' e' 
     possibile inserire anche un livello vuoto
     **
     I consider the actions of the level *inform_tofix->level that make true inform_tofix because is 
     possible to insert an empty level too
  **/
  ...
  if(GpG.input_plan_time_limit>0 && GpG.level_last_fixed_action >=level)
	    return (num_neighborhood);
...
int define_restricted_neighborhood (register FctNode_list node_tofix, int initialize)
{
  ...
  **/
  if (GpG.derived_predicates) 
	...
	create_gdp_path_for(fact_pos, level, GET_ACTION_POSITION_OF_LEVEL(level), &gdp_path);

	if (GpG.derived_pred_in_preconds) //Se pruning attivato (predicati derivati ricorsivi)
  ...
  /**
     Inseriamo l'eventuale azione che blocca la catena di noop che supporterebbero l'inconsistenza in 
     questione
     **
  ...
  **/
  ...
  for (next_level = level - 1; next_level >= 0; next_level--)
    { 
      if (!(CHECK_NOOP_POS (fact_pos, next_level)
	 && check_mutex_noop (fact_pos, next_level) < 0))
      break;		// non posso scendere ulteriormente
      
      
      if(GpG.input_plan_time_limit>0 && GpG.level_last_fixed_action >= next_level)
	break;
    }
  best_action = -1;
  best_all_action = -1;
  ...
  for (j = 0; j < gft_conn[fact_pos].num_A; j++)
	 ...
	 **
	 I choose the best level for the correspondant action and I insert it in the neighborhood
      **/
      total = 0.0;
		...
		if(GpG.level_restrict_neighb == FALSE)
		  ...
		  **/
		  /**
		     ...
		     uguale del costo della migliore azione finora esaminata
		     **
		     If the cost of the action A that we are examinming is lower or equal
		     to the cost of the best action so far examined
		  **/
		  if (best_n_cost.weight >= cost)
		    {
		      loc_weight_cost = weight_cost (&loc_n_cost);
			...
			{
			  /**
			     A parita' di weightcost (costo di inserimento esaminiamo la combinazione di 
			     inserimento, tempo e costo di esecuzione)
			     **
			  ...
			  **/
			  if (best_weight_cost == loc_weight_cost)
			    {
			      /*
				ind_sameact_mincost e' un indice che serve a fare inserire al piu' 
				max_lev_neighb_act+1 canditati di A1. All'inizio vale 0. Dopo avere 
				esaminato la prima azione A1 varra' num_neighb_act(A1) ecc...
				(num_neigh_act e' l'indice del vettore degli elementi del vicinato quindi 
				cresce sempre piu')
			      */
				...
				{
				  /**
				     inserisco l'azione nel vicinato insieme alle altre azioni di tipo A 
				     avente pari costo di inserimento e weight cost
				     **
				     Inserting the action in the neighborhood with the other actions of 
				     type A with the same cost of inserting and weight
				  **/
				  elem_neighb_action[num_neighb_act] = cel;
				  elem_neighb_level[num_neighb_act] = curr_level;
				  elem_neighb_cost[num_neighb_act] = cost;
			  ...
			  **/
		      ...
		      /**
			 Se l'azione migliora cancelliamo tutti gli elementi della medesima azione finora 
			 inseriti e inseriamo l'azione in questione nel vicinato
			 **
			 If the action improve, we remove all the elements of the same action so far 
			 inserted and we insert this action in the neighborhood
		      **/
		      ...
		      /**
			 salviamo la migliore azione tra quelle di tipo cel per la restrizione verticale 
			 del vicinato
			 **
			 we save the best action among those of cel type for the vertical restriction of 
			 the neighborhood
		      **/
		      best_action = cel;
		      best_level = curr_level;
		      ...
		      /**
			 salviamo la migliore azione tra quelle di tipo cel per la restrizione orizzontale 
			 del vicinato
			 **
			 we save the best action among those of cel type for the orizzontal restriction of 
			 the neighborhood
		      **/
		      if (not_tabu(best_action) && (best_all_searchcost > best_n_cost.weight || (best_all_searchcost ==  best_n_cost.weight && best_all_weightcost > best_weight_cost)))
			{
...
#endif
		  /**
		     se il costo dell'elemento supera di 3 volte il costo della migliore azione tra tutte 
		     quelle inserite nel vicinato non la inseriamo
		     **
		     if the cost of the element exceeds 3 times the cost of the best action among those 
		     inserted in the neighbourhood we don't insert it
		  **/
		  continue;
		}
	    }   
  ...
  /**
     ...
     **
     I insert as last element of the neighborhood the best action
  **/
...
int fix_unsup_fact (constraints_list unsup_fact, int num)
{
  ...
  GpG.is_lm = 0;	/**
			   ...
			   **
			   LM for default i don't consider the courrent configuration as a local minimum
			**/
	  ...
	  for (num_lev_neighb_act = 0, curr_level = get_prev(level); curr_level > 0; curr_level = get_prev(curr_level))
	    {
	      ...
	      if(GpG.input_plan_time_limit>0 && GpG.level_last_fixed_action >= curr_level)
		continue;
...
void create_neighborhood_for_compvar (int index, Bool Sign, Bool MulOrDiv,action_set * neighbors, int start, int level)
{
  ...
  float diff;
  Bool add = FALSE;
  Bool try_increase = FALSE;

  if (neighb_actions == NULL)
    neighb_actions =(int *) calloc (gnum_ef_conn / 32 + 1, sizeof (int));
  if (index < 0)
    {
#ifdef __TEST__
  ...
  **/
  ...
  switch (gcomp_var[index].l_operator)
    {
    case EQUAL_OP:
	...
	break;
      ...
      else if (in_vect[gcomp_var[index].first_op] > in_vect[gcomp_var[index].second_op]) {
	  ...
	  if (GET_BIT (neighb_actions, i))
		 ...
		 **
	      ...
	      **/
	      ...
	      val_before = out_vect[gcomp_var[index].first_op] - out_vect[gcomp_var[index].second_op];
	      /**
		 ...
		 **
	      ...
	      **/
		  ...
		  {
		    try_num_eff_in_level(gef_conn[i].numeric_effects[j].index, in_vect, out_vect,TRUE);
		    
		    if (gef_conn[i].numeric_effects[j].lval == gcomp_var[index].first_op)
		      {
			if (!GET_BIT(gis_inertial, gcomp_var_effects[gef_conn[i].numeric_effects[j].index].second_op))
			  {
			    add = TRUE;
			    break;
			  }  
	      ...
	      if (!add)
		{
		  out_vect[gcomp_var[index].first_op]=
		    ri_eval_comp_var (&gcomp_var[gcomp_var[index].first_op], gcomp_var[index].first_op, out_vect, out_vect, TRUE);
		  out_vect[gcomp_var[index].second_op] =
		    ri_eval_comp_var (&gcomp_var[gcomp_var[index].second_op], gcomp_var[index].second_op, out_vect, out_vect, TRUE);
	      ...
	      if (add)
		{
	      ...
	      else if (gcomp_var[index].l_operator== EQUAL_OP) 
		{
		  if ((try_increase && (diff > MAX_APPROX))
		      || (!try_increase &&(diff < -MAX_APPROX)))
		    {
		       neighbors->A[neighbors->num_A++] = i;
		    }
		}
	      else if (((gcomp_var[index].l_operator== LESS_THAN_OP) || (gcomp_var[index].l_operator== LESS_THAN_OR_EQUAL_OP))
		  && (diff < -MAX_APPROX))
		{
		  neighbors->A[neighbors->num_A++] = i;		 
		}
	      else if (((gcomp_var[index].l_operator == GREATER_THAN_OP) || (gcomp_var[index].l_operator == GREATER_OR_EQUAL_OP))
		  && (diff > MAX_APPROX))
		{
		  neighbors->A[neighbors->num_A++] = i;		
		}
...
void create_remotion_neighborhood_for_compvar (int fct_pos, int level)
{
  ...
  **/
  ...
  for (indlevel = get_prev(level); indlevel >= 0; indlevel = get_prev(indlevel)) {
    ...
    found = FALSE;
    switch (gcomp_var[fct_pos].l_operator) 
      {
      case EQUAL_OP:
	// MODIFICATO X UGUALIANZA
	...
	*/
    ...
    if (found && does_action_remotion_support_numeric_precond_in_down_level(fct_pos, level, indlevel))
      {
	insert_element_in_neighb (&temp_act);
      }
  ...
  for (i = GpG.num_false_fa - 1; i >= 0; i--)
      ...
      else if (GpG.inc_choice_type == MIN_LEVEL_MIN_TIMED_INC)
	{
	  if (min_level > *unsup_fact[i]->level)
		 ...
		 **
		 Calculating the time fact more restrictive for the examined inconsistence
	      **/
		 ...
		 **
		 Calculating when the inconsistence can be supported
	      **/
		 ...
		 **
		 Calculating the slack of the examined inconsistence
	      **/
...
constraints_list choose_min_level_inconsistence()
{
  ...
  /**
     ...
     **
     Scanning the logical inconsistence list
  **/
  for (i=0;i<GpG.num_false_fa;i++)
    {
	if (min_level > *unsup_fact[i]->level)
  ...
  for (i=0;i<GpG.num_false_num_fa;i++)
      ...
      else if (min_cost == cost)
	{
	  same_cost_inconsistence[num_same_cost++] = unsup_num_fact[i];
	    ...
	    Se viene superato il limite massimo dell'array, rialloco aggiungendo spazio
	    **
	    If the array exceeds the maximum limit, I reallocate adding space
	 **/
	  if (num_same_cost>=num_max_vector_inc){
            num_max_vector_inc+=MAX_VECTOR_INC;
	    if (!(same_cost_inconsistence = (constraints_list *)realloc(same_cost_inconsistence,num_max_vector_inc*sizeof(constraints_list)))) MSG_ERROR(WAR_NO_MEMORY);//out of memory
...
constraints_list choose_min_level_cost_inconsistence()
{
  ...
  static int num_max_vector_same_level_inc=MAX_VECTOR_INC;/**
							     Tiene il numero max di elementi dell'array al 
							     livello minimo
							     **
							     maximum number of array's elements at the 
							     minimum level
							  **/
  static int num_max_vector_same_cost_inc=MAX_VECTOR_INC;/**
							    Tiene il numero max di elementi dell'array per 
							    il costo minimo
							    **
							    maximum number of array's elementsfor the 
							    minimum cost
							 **/
  int i,best;
  int num_same_level=0;
	...
	if (min_level == *treated_c_l[i]->level)
	  {
	    same_level_inconsistence[num_same_level++]=treated_c_l[i];
	       ...
	       Se viene superato il limite massimo dell'array, rialloco aggiungendo spazio
	       **
	       If the array exceeds maximum limit, I reallocate adding space
	    **/
	    if (num_same_level>=num_max_vector_same_level_inc)
	      {
		num_max_vector_same_level_inc+=MAX_VECTOR_INC;
  ...
  **/
  for (i=0;i<num_same_level;i++)
	...
	/**
	   ...
	   **
	   if it is a threated, the flag of presence is activated
	**/
	treated_flag=1;
	continue;
      }
      else
	{
	  /**
	     ...
	     **
	     else it is a fact, then the cost is evaluated
	  **/
	  /**
	     Calcolo del costo
	     **
  ...
  **/
      ...
      if( GpG.neighb_with_timed_fa && fix_timed )
	{
	  /**
	     Risolvo prima le inconsistenze temporali
	     **
	     First resolving temporal inconsistence
	  **/
	  if (GpG.num_false_tmd_fa > 0) {
	    //	    printf("T");
...
int search_step(int num_try, int min_time, int numrestart, PlanAction ** plan_actions, State * start_state, State * end_state)
{
  ...
  /** 
      ...
      - nel caso in cui l'ottimizzazione sia attiva o siano gia' state trovate altre soluzioni,si richiama 
        la funzione restart_search () che rimuove un'azione dall'action subgraph in modo casuale al fine di
	introdurre delle nuove inconsist. cosi da continuare il processo di ottimizzazione.
        Se non si presentano ancora delle inconsistenze, viene ritonarto 0 e si esce.
      **
      If the three arrays containing inconsistence are empty, we control if we are optimizing or if there 
      already are other solutions

      - if the optimization is disabled or there is not a solution, we go out to the function and return 0
      - if the optimization is abled or there are solutions, we call the restart_search() function that 
        randomly remove an action to the action subgraph to introducing new inconsistences to continue the 
  ...
  if (GpG.num_false_fa <= 0 && GpG.num_false_act <= 0 && GpG.num_false_num_fa <= 0)
	...
	//	if ( !GpG.neighb_with_timed_fa || (GpG.neighb_with_timed_fa && GpG.num_false_tmd_fa <=0 ))
	{
	  if(GpG.timed_facts_present)
		...
		{
		  GpG.qs_best_cost = GpG.total_cost_from_metric;
      ...
      for (num_restart = 0; num_restart < GpG.numrestart; num_restart++)
	{
	  ...
	  **/
	  ...
	  for (num_try = 1, num_try_print = 1, try_time_check = 1; num_try < tot_numtry;
		 ...
		 **
		 every five iteration, I check to not have overtaken max_cputime
	      **/
	      if (try_time_check >= 5)	
		{
	      ...
	      if (GpG.num_false_tot == 0)
		{
	      ...
	      if (GpG.num_false_tot == 0)
		{
		  if(GpG.enable_numeric_compress && GpG.splitted_actions)
		    {
		  ...
		  if (!optimize)
		  ...
		  else
		    {
		      ...
		      **/
		      ...
		      GpG.best_exec_cost = GpG.total_cost;
		      GpG.best_time = GpG.total_time;
		      GpG.best_numact = GpG.num_actions;
		      GpG.best_plan_num_new_actions = GpG.num_new_actions ;
		      GpG.input_plan_lenght = GpG.curr_plan_length;
		    }

		  if ((GpG.enable_numeric_compress && GpG.splitted_actions) || optimize || 
	      ...
	      if(GpG.store_plan)
		{
		  ...
		  **/
		  ...
		  reset_plan_param(newlevel, plan_actions);
			...
			plan_info_for_quality_mode.metricvalue = GpG.total_cost_from_metric;
