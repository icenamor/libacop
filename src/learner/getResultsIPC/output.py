...
def writeFile(data,name):
	...
	for line in data:
		...
		v = 0
		t = 0
		for value, time in zip(line.values, line.oktimesols):
			v = value
			t = time
		name += str(v) + "," + str(t) + ","+str((v > -1)) + "\n"
		##print name
		fd.write(name)
	fd.close()
...
def processData(data, results):
	...
	while(i < len(data) and  data[i].find("+---") < 0):
		...
		p1 = p2 + 1 + p1
		p2 = line[p1:].find("|")
		values = line[p1:p2+p1].strip()
		result.values = result.processVector(values, result.values)
		p1 = p2 + 1 + p1
		p2 = line[p1:].find("|")
		oktimesols = line[p1:p2+p1].strip()
		result.oktimesols = result.processVectorTime(oktimesols, result.oktimesols)
		i = i + 1
