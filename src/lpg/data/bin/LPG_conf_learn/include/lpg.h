...
//#define ROUND_TO_1_10000(f) ((float)((int)((f)*10000.0 + 0.5 ))/10000.0)
 ...
 #define ROUND_TO_1_1000(f) ((float)((int)((f)*10.0 + 0.5 ))/10.0)
#endif
...
#define MAX_GOALS 1000		/* max number of goals in an array; just technical */
...
//MODIFICHE COCCOLI
#define MAX_VECTOR_INC 250 /* Numero massimo degli elementi degli array utilizzati nelle funzioni di rimozione delle inconsistenze*/
 ...
 * Used in H_relaxed
 **/
#define MAX_LENGTH_H 65536

#define UNREACHABLE_ACTION -1
#define INITIAL_ACTION -2
#define GOAL_ACTION -3


#define NOT_MARKED_INPUT_PLAN -4
...
#else
#define MIN_DELTA_TIME 0.025																	      
#endif

#define TOTAL_TIME_FUNCTION_INDEX 1


#define NUMERIC_THREATS_NUM 0
#define NUMERIC_THREATS_MAX 1
#define NUMERIC_THREATS_TOT 2
...
typedef struct _NODE_COST
{
  float weight;
  float input_plan_cost;
  float act_cost;
  float act_time;
  float timed_fa;
  int num_actions;
 ...
 **/
typedef struct _NOOP_NOT_IN
{
  int position;
  struct _NOOP_NOT_IN *next;
}
noop_not_in;
...
{ 
  ...
  **/
  ...
  /** ADAPT
   * TRUE if thecurrent action belongs to the input plan
   **/
     Bool input_plan_act;

  /*
    TRUE if this action can not be removed during the search
  */
  Bool fixed_action;
...
typedef struct _HVAR
{
  ...
  int *ri_num_actions_of_numeric_facts;
  ...
  int *temp_applied;
  int *input_plan_actions;

  int *numeric_precs_in_relaxed_plan;
...
extern struct _NUMERIC  Numeric;
...
typedef struct _GPG
   ...
   **/ 
   ...
   int curr_min_time,  curr_plan_length,  init_plan_length,  max_plan_length,
     ...
     **/
    float total_cost,  best_cost, best_exec_cost;
    ...
    float weight_cost,  orig_weight_cost,  weight_time,  orig_weight_time, orig_weight_timed_fa, weight_timed_fa,weight_input_plan_cost;
    ...
    Bool improve_reachability;
    
    Bool derived_pruning_on;  

    Bool recursive_rules;

    float pruning_weight;

    Bool disable_pruning;

    Bool enable_numeric_compress;
    ...
    int orig_input_plan_num_actions ;
    int *input_plan_actions;
    int *orig_input_plan_actions;

    int num_input_plan_max_changes;
    int num_new_actions;
    int best_plan_num_new_actions;
    int optimize_plan_differences;
    int optimize_differences_quality;
    int input_plan;
    float optimize_differences_quality_coefficient;
    int weight_num_solutions;
    int best_act_penalization_coeff;
    int adapt_all_diff;
  /* ADAPT */

    Bool perform_split;

    short numeric_threats_mode;
    
    int last_action_inserted_removed_level;
    int level_last_fixed_action;
    float  input_plan_time_limit;
    float  input_plan_start_time;
    float  input_plan_end_time;
    float  input_plan_end_insertion_time_limit;
    int    not_insert_time_limit_actions; // 0: insert actions included in the time limit (default); 1: NOT insert the actions  included in the time limit; 2: insert actions included in the time limit which start after input_plan_end_insertion_time_limit
    float  input_plan_percentage_limit;

    int num_marked_input_plan_act;
    int num_not_marked_input_plan_act;
    int reschedule_input_plan;

    struct _ACT_NODE *end_time_action_f;
    
    float first_solution_cpu_time;

} _GpG;

extern struct _GPG GpG;
...
typedef struct _STAT
{
  ...
  struct tms PD_start_time, PD_end_time ;
  int PD_time_flag;
} _Stat;
...
typedef struct   _PLAN_INFO_FOR_QUALITY_MODE
{
  int num_actions;
  float total_cost;
  float total_time;
  float metricvalue;
}  _plan_info_for_quality_mode;

extern struct _PLAN_INFO_FOR_QUALITY_MODE  plan_info_for_quality_mode;

typedef struct  _LOCAL_SEARCH
{
  int best_action;
  int best_level;
  int ls_continue;
...
} _local_search; // Global variables for the local search phase 

extern struct _LOCAL_SEARCH local_search;



typedef struct _LAST_ACT_COST 
{
  int step;
  int level;
  char saved_prec;
...
typedef struct _CONDITIONAL_INFO
{
...
typedef struct __HASH_FOR_GEFCONN
{
  int ef_pos;
  struct __HASH_FOR_GEFCONN* next;
} __hash_for_gefconn;
...
extern int value_start_size;
extern float  *gcomp_var_value_start;
extern int temp_value_size;
...
extern int *gnum_tmd_interval_block;

extern int **gtimed_fct_bitarray;
...
extern token_list inertial_facts;

extern IntList *old_list;

extern float *val_max, *val_min;

// Functions of time.c


int propagation_time (ActNode_list infAction, float tolerance);
