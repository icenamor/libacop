...
int propagation_time (ActNode_list infAction, float tollerance)
{
  ...
  float max_time, savetime, toll, dur;
  int level, next_level, ind;
  EfConn *act;
  double time_f , diff_f;
 ...
 */
  ...
  **/
  ...
  if(gef_conn[infAction->position].act_type == END_SPLITTED)
	...
	dur=get_action_time (infAction->position, level) ;

	if (infAction->time_f == ( max_time + dur +toll))
	  return (0);
	else
	  {
	    if(toll)
		...
		time_f = ( (max_time+ dur ) + toll ) ;
		
		infAction->time_f = ((float) time_f);
		
		diff_f= (time_f - (double) infAction->time_f);
#ifdef __TEST__
		
		if(toll)
		  {
		    printf("\n Act %s , pos %d  time_start %.5f max_time %.5f dur %.5f sumtimef %.5f toll %.5f timef %.5f diff %.5f ", print_op_name_string(infAction->position,temp_name), infAction->position, time_f - dur , max_time,dur,  infAction->time_f  ,  toll, time_f, diff_f );
		  }
#endif
		...
		/*****
		 ATTENZIONE: Check per rimediare a STRANI problemi di tolleranza in alcuni domini!!!!
		 WARNING: check to avoid tolerance errors!!
		****/
		if( diff_f>0.0001 || (-diff_f)> 0.0001)
		  {
		    ...
		    if(toll)
		      printf("\n ACT %s , pos %d  time_start %.4f max_time %.4f dur %.4f sumtimef %.4f toll %.4f timef %.4f diff %.4f ", print_op_name_string(infAction->position,temp_name), infAction->position, time_f - dur , max_time,dur,  infAction->time_f  ,  toll, time_f, diff_f );
#endif
		  }
	      }
	    else
	       infAction->time_f = max_time + dur;
	  
	  }
      }
  if ((GpG.end_time_action_f == NULL) || (GpG.end_time_action_f->time_f < infAction->time_f))
    GpG.end_time_action_f = infAction;
  ...
  */
  for (ind = level + 1; ind < GpG.curr_plan_length; ind++)
	 ...
	 **
	 ...
	 its time
      **/
      propagation_time (GET_ACTION_OF_LEVEL (ind),0.0);
      //  propagation_time (GET_ACTION_OF_LEVEL (ind),TIME_TOLERANCE);
...
void get_total_time_plan ()
{
  ...
  if (max_time < 0)
    {
      max_time = GpG.end_time_action_f->time_f;
    }
...
void compress_numeric_plan()
{
  ...
  if (DEBUG2)
    printf("\n\n\nCompress Numeric Plan: (SOL %d RUN %d)", GpG.num_solutions, GpG.count_num_try);

  temp_act = GpG.tempplan;
  i = 0;
  while (temp_act)
    {
      ...
      for (j = i - 1; j >= 0; j--)
	{
      ...
      if (DEBUG2)
	printf ("\nInitialize->insert action [%d] %s  in level %d (max_time = %.2f)",
		position, print_op_name_string (position, temp_name), j + 1, max_time);
  ...
  for (i=0; i < gnum_timed_facts; i++)
    {
      ...
      memset (gtimed_fct_bitarray[i], 0, gnum_tmd_interval_block[i]);
	      ...
	      if(is_fact_in_preconditions_overall(infAction->position, timedFct->position) || is_fact_in_preconditions_end(infAction->position, timedFct->position))
		{
		  if (timedFct->end_time - infAction->time_f > 0)
		      ...
		      if (timedFct->slack > slack_vect[*infAction->level] + timedFct->end_time - infAction->time_f)
			{
			  ...
			  SET_BIT (gtimed_fct_bitarray[i], j);
			}
		    }
		}
	      else
...
void update_end_time_action_f(void) 
{
  ...
  for (i = 0; i < GpG.curr_plan_length; i++)
    {
      if (vectlevel[i]->action.position < 0)
	continue;

      if (vectlevel[i]->action.time_f > max_time)
	{
	  max_time = vectlevel[i]->action.time_f;
	  GpG.end_time_action_f = &vectlevel[i]->action;
	}
    }
}
