...
void store_pop(char *fact_file_name)
{
   ...
   if (GpG.out_file_name)
      sprintf (cNameFile, "%s", gcmd_line.out_file_name, num_plan);
   else
...
void recompute_start_times_with_tolerance()
{
  ...
  */
  for (ind = 0; ind < GpG.curr_plan_length; ind++)
    {
      // Force its recomputation
      prop_level_index[ind]=1;
      if(GET_ACTION_POSITION_OF_LEVEL (ind)<0)
	continue;
	 ...
	 **
      ...
      **/
      //      propagation_time (GET_ACTION_OF_LEVEL (ind),TIME_TOLERANCE);
       propagation_time (GET_ACTION_OF_LEVEL (ind),0.0002);
...
store_temporal_plan (int levels, char *fact_file_name, double time)
{
  ...
  recompute_start_times_with_tolerance();
  ...
  if (GpG.out_file_name)
      ...
      if (GpG.mode != SPEED && GpG.mode != QUALITY)
	{
	  ...
	  sprintf (cNameFile, "%s", gcmd_line.out_file_name, num_plan);
#else
	  sprintf (cNameFile, "%s%s", gpath_sol_file_name, gcmd_line.out_file_name, num_plan);
#endif
	}
      else
	{
#ifndef __WINLPG__
	  sprintf (cNameFile, "%s", gcmd_line.out_file_name);
#else
	  sprintf (cNameFile, "%s%s", gpath_sol_file_name, gcmd_line.out_file_name);
#endif
	}
    }
  else
 ...
 // fprintf (fp, "\n; Command line: %s", gcomm_line);
  ...
  if ( GpG.search_type == LOCAL)
   ...
   if(GpG.num_solutions==1)
     {
       GpG.first_solution_cpu_time=gtotal_time;
       printf("\n first_solution_cpu_time: %.2f ",gtotal_time);
     }
   fprintf (fp, "\n; Time %.2f", gtotal_time);
  ...
  if (time >= 0.0)
	...
	//	if (DEBUG2)
	      ...
	      else
		{
		  ...
		  start_act_time = (infAction->time_f - get_action_time (infAction->position, *infAction->level));
...
#ifdef __TEST__
	      fprintf (fp, ";; Index=%d", infAction->position);
	      fprintf (fp, ", are mutex 31 vs %d = %d -- %d",infAction->position, ARE_MUTEX_TRI_EF(31,infAction->position),ARE_MUTEX_TRI_EF(infAction->position,31) );
...
/*#ifdef __MY_OUTPUT__
  ...
  fprintf (fp, "; Time %.2f", gtotal_time);
...
void print_solution_time_and_cost(void)
{
  ...
  if (GpG.maximize_plan && GpG.total_cost_from_metric < 0)
    printf("\n Found Solution: TimeExec %.2f TimeCost %.2f TotWeightCost %.2f",
	   GpG.total_cost, GpG.total_time, GpG.total_cost_from_metric);
  else
    printf("\n Found Solution: TimeExec %.2f TimeCost %.2f TotWeightCost %.2f",
	   GpG.total_cost, GpG.total_time, GpG.total_cost_from_metric);
...
void print_solution_features(float plan_time, int num_restart)
{
  if (GpG.mode == INCREMENTAL)
	...
	GpG.num_solutions, gtotal_time, plan_time, GpG.num_actions, 
	  GpG.total_time, GpG.total_cost_from_metric,GpG.count_num_try);
      else
	{
	  printf ("\nSolution number: %d\nTotal time:      %.2f\nSearch time:     %.2f\nActions:         %d\nDuration:        %.3f\nPlan quality:    %.3f \nTotal Num Flips: %d", 
	GpG.num_solutions, gtotal_time, plan_time, GpG.num_actions, 
  ...
  else if (GpG.mode == SPEED)
    {
      if(GpG.maximize_plan && GpG.total_cost_from_metric < 0)
	printf ("\nSolution found:\nTotal time:      %.2f\nSearch time:     %.2f\nActions:         %d\nDuration:        %.3f\nPlan quality:    %.3f \nTotal Num Flips:  %d", 
	gtotal_time, plan_time, GpG.num_actions, GpG.total_time, 
 	GpG.total_cost_from_metric, GpG.count_num_try   );
      else
	printf ("\nSolution found:\nTotal time:      %.2f\nSearch time:     %.2f\nActions:         %d\nDuration:        %.3f\nPlan quality:    %.3f \nTotal Num Flips:  %d", 
	gtotal_time, plan_time, GpG.num_actions, GpG.total_time, 
	GpG.total_cost_from_metric, GpG.count_num_try ); 
