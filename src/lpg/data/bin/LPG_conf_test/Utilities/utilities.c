...
int is_plan_better ()
{
  ...
  **/
...
#ifdef __TEST_DIFF__     
  diff=((1.0- GpG.optimize_differences_quality_coefficient) * GpG.total_cost_from_metric) / (GpG.best_cost +  (GpG.optimize_differences_quality_coefficient*GpG.num_new_actions)/((float) GpG.best_plan_num_new_actions));
      printf("\n Diff value: %d _ best %d --- quality %f _ best %f --- tot eval %.3f",GpG.num_new_actions,GpG.best_plan_num_new_actions ,GpG.total_cost_from_metric, GpG.best_cost, diff);
#endif
  if(GpG.optimize_plan_differences)   
    ...
    {
      quality= (GpG.total_cost_from_metric) / GpG.best_cost;
  ...
  if ((GpG.best_cost - TIME_TOLERANCE) > GpG.total_cost_from_metric)
    {
      if (DEBUG3)
	printf ("\n\n !!!  Find BETTER solution => SAVE \n");
      return TRUE;
...
*  Call gives: init_act_vect
**/
int get_action_index (char *opname, int *index_of_arg, int num_operands)
{
  /**
    Variabili di appoggio
    **
  ...
  **/
  ...
  for(ptr=local_hash_table[hash_v]; ptr; ptr=ptr->next)
    {
      index=ptr->ef_pos;
  ...
  iter++;
	...
	**/
	 ...
	 **
      ...
      **/
      act_pos = get_action_index (opname, index_of_arg, num_operands);
      /**
...
void initialize_preset_values (void)
{
  ...
  GpG.first_solution_cpu_time=0.0;
...
Bool process_command_line (int argc, char *argv[])
{
  ...
  for (i = 1; i < argc; i++)
	...
	sscanf (argv[++i], "%f", &GpG.k_weight_false_fa);
      else if (strcmp (argv[i], "-k2") == 0)
	sscanf (argv[++i], "%f", &GpG.k_weight_false_num_fa);
      else if (strcmp (argv[i], "-k3") == 0)
	sscanf (argv[++i], "%f", &GpG.k_weight_false_act);
      else if (strcmp (argv[i], "-k4") == 0)
	sscanf (argv[++i], "%f", &GpG.k_weight_false_tmd_fa);
...
void insert_extended_unsupported_facts_for_action(int action, int level)
{
  ...
  for (i = 0; i < gef_conn[action].num_PC; i++)
    {
      if( Hvar.num_extended_unsupported_facts >= MAX_VECTOR_INC )
	return;
